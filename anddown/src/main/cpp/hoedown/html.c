...
rndr_blockcode(hoedown_buffer *ob, const hoedown_buffer *text, const hoedown_buffer *lang, const hoedown_renderer_data *data)
{
	...
	if (lang) {
		HOEDOWN_BUFPUTSL(ob, "<pre style=\"max-width: 100%;\" ><code class=\"language-");
...
{
	if (ob->size) hoedown_buffer_putc(ob, '\n');
	HOEDOWN_BUFPUTSL(ob, "<blockquote style=\"max-width: 100%;\" >\n");
...
{
	HOEDOWN_BUFPUTSL(ob, "<code style=\"max-width: 100%;\" >");
...
{
	if (ob->size) hoedown_buffer_putc(ob, '\n');
	hoedown_buffer_put(ob, (const uint8_t *)(flags & HOEDOWN_LIST_ORDERED ? "<ol style=\"max-width: 100%;\"  >\n" : "<ul style=\"max-width: 100%;\" >\n"), 5);
	if (content) hoedown_buffer_put(ob, content->data, content->size);
...
rndr_paragraph(hoedown_buffer *ob, const hoedown_buffer *content, const hoedown_renderer_data *data)
{
	...
	HOEDOWN_BUFPUTSL(ob, "<p style=\"max-width: 100%;\" >");
...
{
    if (ob->size) hoedown_buffer_putc(ob, '\n');
    HOEDOWN_BUFPUTSL(ob, "<table style=\"max-width: 100%;\" >\n");
    hoedown_buffer_put(ob, content->data, content->size);
...
rndr_footnotes(hoedown_buffer *ob, const hoedown_buffer *content, const hoedown_renderer_data *data)
{
	...
	if (ob->size) hoedown_buffer_putc(ob, '\n');
	HOEDOWN_BUFPUTSL(ob, "<div style=\"max-width: 100%;\"  class=\"footnotes\">\n");
	hoedown_buffer_puts(ob, USE_XHTML(state) ? "<hr/>\n" : "<hr>\n");
