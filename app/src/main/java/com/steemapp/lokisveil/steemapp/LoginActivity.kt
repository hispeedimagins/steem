...
class LoginActivity : AppCompatActivity() , GlobalInterface{
 ...
 /*           var followerlistener: Response.Listener<JSONObject> =  Response.Listener { response ->
                ...
                if(parse != null && parse.result != null){
                    ...
                    FollowApiConstants.getInstance().followers.addAll(parse.result as List<prof.Resultfp>)
                    if(FollowApiConstants.getInstance().followers.size == FollowApiConstants.getInstance().followCount.result.followerCount){
                        var fold = FollowersDatabase(applicationContext)
                        var alldbpeople = fold.GetAllQuestions()
                        for(x in FollowApiConstants.getInstance().followers){
                            if(fold.Insert(x)){
