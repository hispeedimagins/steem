...
public class LinuxSecureRandom extends SecureRandomSpi {
    ...
    static {
        try {
            ...
            // Now override the default SecureRandom implementation with this
            // one.
            int position = Security.insertProviderAt(new LinuxSecureRandomProvider(), 1);

            if (position != -1)
                Log.i("position","Secure randomness will be read from  only");
            else
                 Log.i("position","Randomness is already secure.");
        } catch (FileNotFoundException e) {
            // Should never happen.
                Log.e("position","/dev/urandom does not appear to exist or is not openable");
            throw new RuntimeException(e);
        } catch (IOException e) {
            Log.e("position","/dev/urandom does not appear to be readable");
            throw new RuntimeException(e);
        }
    }
