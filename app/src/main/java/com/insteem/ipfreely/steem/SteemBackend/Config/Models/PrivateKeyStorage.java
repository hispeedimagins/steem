...
import com.insteem.ipfreely.steem.Crypto.Base59.Sha256ChecksumProvider;
import com.insteem.ipfreely.steem.Crypto.DumpedPrivateKey;
import com.insteem.ipfreely.steem.Crypto.ECKey;
...
public class PrivateKeyStorage {
    ...
    public void addPrivateKeyToAccount(AccountName accountName, ImmutablePair<PrivateKeyType, String> privateKey) {
        /*Map<AccountName, List<ImmutablePair<PrivateKeyType, ECKey>>> keys = this.getPrivateKeysPerAccounts();
        List<ImmutablePair<PrivateKeyType, ECKey>> acc = keys.get(accountName);
        ImmutablePair<PrivateKeyType, ECKey> adk = convertWifToECKeyPair(privateKey);*/
        //acc.add(adk);
